...
public class playerControl : MonoBehaviour
{
    ...
    { 
        ...
        for (int i = 0; i < 17; i++)
        {
            bone_pos.Add(new Vector3(-1 * tryonnx.output3D[0, frame / rate, i, 0], -1*tryonnx.output3D[0, frame / rate, i, 1], -1 * tryonnx.output3D[0, frame / rate, i,2]));
            if (ff == 0)
            {
                Debug.Log(-1 * tryonnx.output3D[0, frame / rate, i, 0] + ", " + -1*tryonnx.output3D[0, frame / rate, i, 1] + ", " + -1 * tryonnx.output3D[0, frame / rate, i, 2]);
            }
        }
        ff++;
        updatePose(bone_pos);
