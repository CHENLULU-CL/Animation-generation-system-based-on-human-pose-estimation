...
public class player : MonoBehaviour
{
    ...
    //int[] parent = new int[] { -1, 0, 1, 2, 0, 4, 5, 0, 7, 8, 9, 7, 11, 12, 7, 14, 15 };
    private Vector3 initPos;
    //动画相关
    Animator animator;
    //public Transform nose;
    private Transform root, spine, neck, head, leye, reye, lshoulder, lelbow, lhand, lthumb2, lmid1, rshoulder, relbow, rhand, rthumb2, rmid1, lhip, lknee, lfoot, ltoe, rhip, rknee, rfoot, rtoe, chest;
    ...
    void Update()
    {
        List<Vector3> bone_pos = new List<Vector3>();

            for (int i = 0; i < 17; i++)
            {
                bone_pos.Add(new Vector3(-10 * tryonnx.output3D[0, frame / rate, i, 0], -10 * tryonnx.output3D[0, frame / rate, i, 1], -10 * tryonnx.output3D[0, frame / rate, i, 2]));
                //if (ff == 0)
                //{
                //    Debug.Log(tryonnx.output3D[0, frame / rate, i, 0] + ", " + -1 * tryonnx.output3D[0, frame / rate, i, 1] + ", " + tryonnx.output3D[0, frame / rate, i, 2]);
                //}
            }
            ff++;
        if (frame % rate == 0)
    ...
    void updatePose(List<Vector3> pred3D)
    {
        ...
        if (frame == 0)
        {
            //initPos = new Vector3(pred3D[0].x, -1 * pred3D[0].y, pred3D[0].z);
            initPos = new Vector3(pred3D[0].x, -1 * pred3D[0].y, pred3D[0].z);
        }
        /// 
        //////////////////////  更新位置 //////////////////////
